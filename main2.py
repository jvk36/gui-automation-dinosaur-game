...
def detect_obstacle(game_area):
    ...
    dark_pixel_threshold = 100  # Adjust if necessary
    
    # Count how many pixels are below this threshold (darker regions)
    dark_pixel_count = np.sum(image_array < dark_pixel_threshold)
    
    # print(f"Dark pixel count: {dark_pixel_count}")  # Print for debugging
...
def play_game():
    ...
    while True:
        ...
        if detect_obstacle(game_area) > 1200:  # Adjust the threshold as needed
            jump()
